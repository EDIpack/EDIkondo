...
contains
  ...
  subroutine ed_read_input(INPUTunit)
    ...
    call parse_input_variable(Vkondo,"Vkondo",INPUTunit,default=0.d0,comment="V_kondo: (Vdir-Jk_z/4) direct exchange appearing in a real atomic system Vk*sum_rs n_b,r*n_imp,s")
    !
    dim=Nsites(Norb+1)
    allocate(Jkindx(dim))
    allocate(Cindx(dim))
    ...
    !
    allocate(gf_flag(Norb+1))
    allocate(chispin_flag(Norb+1))
