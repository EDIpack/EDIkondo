...
contains
  ...
  subroutine ed_checks_global
    ...
    !
    if(ed_twin)then
       write(LOGfile,"(A)")"WARNING: can not yet use twin_sector for the Kondo Problem yet. Set to false."
       ed_twin=.false.
    endif
    !
  ...
  subroutine setup_global
    ...
    do isector=1,Nsectors
       ...
       Jups(1)=Jups(1)-1;
       if(Jups(1) < 0)cycle
       if((Jups(1) <= 0) .AND. (Jdws(1)<=0) )cycle
    ...
    do isector=1,Nsectors
       ...
       if((Jups(1) >=eNs+Nimp) .AND. (Jdws(1) >=eNs+Nimp) )cycle
       call get_Sector([Jups,Jdws],Ns,jsector)
